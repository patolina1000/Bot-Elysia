...
export async function getSettings(botSlug: string): Promise<BotSettings> {
  const result = await pool.query(
    'select bot_slug, pix_image_url, offers_text from bot_settings where bot_slug = $1 limit 1',
    [botSlug]
  );
  ...
  return { bot_slug: botSlug, pix_image_url: null, offers_text: null };
}

export async function saveSettings(
  botSlug: string,
  settings: { pix_image_url?: string | null; offers_text?: string | null } = {}
): Promise<BotSettings> {
  const pixImageUrl = settings.pix_image_url ?? null;
  const offersText = settings.offers_text ?? null;

  const result = await pool.query(
    `insert into bot_settings (bot_slug, pix_image_url, offers_text)
     values ($1, $2, $3)
     on conflict (bot_slug) do update
       set pix_image_url = excluded.pix_image_url,
           offers_text = excluded.offers_text,
           updated_at = now()
     returning bot_slug, pix_image_url, offers_text`,
    [botSlug, pixImageUrl, offersText]
  );
